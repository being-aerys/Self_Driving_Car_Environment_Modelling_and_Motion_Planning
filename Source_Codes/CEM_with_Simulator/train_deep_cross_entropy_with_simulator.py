...
class DeepCEM:
    ...
    def generate_episode_using_simulator(self, t_max=100):
        """
    ...
    def train_CEM(self):
        ...
        for generation in range(self.N_GENERATIONS):
            ...
            episodes_info = [self.generate_episode_using_simulator(self.N_TSTEPS_HORIZON) for episode in range(self.N_EPISODES)]
            if self.FIRST_RUN:
                self.FIRST_RUN = False

            states_batch, actions_batch, rewards_batch = map(np.array, zip(*episodes_info))
...
if __name__ == "__main__":
    ...
    deep_cem = DeepCEM() # just object instantiation trains CEM in the init() method.
